...
*   **任務:**
    ...
    2.  **前端整合:**
        *   建立 `DAO 治理` 頁面，顯示所有提案的列表與狀態（投票中、已批准、已拒絕）。
        *   實作投票介面，讓 DAO 成員可以對投票中的提案進行投票。
        *   在募資專案卡片上，明確顯示該專案是否已獲得「DAO 官方認證」的標章。
    3.  **部署腳本更新:**
        *   更新部署腳本 (`Deploy.s.sol`)，使其能正確部署 `EduDAO` 和 `FundraiserFactory` 合約，並將前者的地址注入後者。
    4.  **(進階/可選) 去中心化儲存整合:**
